...
class MCTS:
                ...
                if (current_node, action) not in self.state_transition_table:
                    if isinstance(self.model, MuZeroNetwork):
                        ...
                        try:
                            ...
                            if isinstance(current_node.hidden_state, torch.Tensor) and current_node.hidden_state.dtype != torch.float32:
                            ...
                            if isinstance(current_node.hidden_state, torch.Tensor):
                                ...
                                new_state = torch.zeros(shape, device=device, dtype=torch.float32)  # Explicitly set dtype
                            else:
                                new_state = np.zeros_like(current_node.hidden_state, dtype=np.float32)  # Explicitly set dtype
                    else:
    ...
    def _calculate_ucb_score(self, node: Node, action: Action, visit_count: int, total_visit_count: int) -> float:
        ...
        if (node, action) not in self.policy_table:
            ...
            self.policy_table[(node, action)] = 1.0 / len(node.get_available_actions()) if node.available_actions else 0.5
    ...
    def _expand_node(self, node: Node) -> float:
        ...
        if isinstance(policy, torch.Tensor):
            ...
            policy_dict = {Action(a): float(p.item()) for a, p in zip(range(self.config.ACTION_SPACE), policy_cpu.flatten())}
